...
try:
    with open("core/artifact/quill/tool-results.json", 'w') as index_output_f:
        index_output_f.write('[\n')

        for gear in card_data:
            if gear["equipment_category"]["index"] == "tools":
                try:
                    index_output_f.write('{')
                    index_output_f.write('"slug":"' + slugify(gear["name"]) + '",\n')
                    index_output_f.write('"name":"' + gear["name"] + '",\n')
                    index_output_f.write('"type": {"slug": "tool","name": "Tool"},\n')
                    ...
                    try:
                        index_output_f.write('"tool_category": {"slug":"' + slugify(gear["tool_category"]) + '", "name":"' + gear["tool_category"] + '"},\n')
                    except:
                        index_output_f.write('"tool_category": false,\n')
